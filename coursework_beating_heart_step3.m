...
[m n] = size(pos_beat_trajectory); %obtain matrix size
...
for L=1:1:n-1 %loop to calculate distance and time variables
    ...
    %stores in an array
    distance(L) = sqrt((pos_beat_trajectory(1,L)- pos_beat_trajectory(1,L+1))^2 + (pos_beat_trajectory(2,L)- pos_beat_trajectory(2,L+1))^2);
end
...
for i=1:1:n %for loop to set trajectory points
    ...
    %points
    point(i,:) = [pos_beat_trajectory(1,i); pos_beat_trajectory(2,i); 0]; 
